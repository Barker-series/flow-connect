 ...
 * Boot warming.
 *
 * FLOW CONNECT ships no image files: every texture is generated from the Canvas2D
 ...
 * the stylesheet's flat stage colour is the fallback.
 */
    ...
    try {
        ...
        if (!composed) {
            composed = {
                ...
                // The board hangs in the middle of the screen: the header owns
                // the top, the controls are anchored to the bottom, and the
                // tubes belong in the open space between them.
                ...
                // Landscape hangs the board under the TITLE, in the left
                // column: the controls own the right, and tubes behind buttons
                // are tubes nobody reads.
        ...
        console.warn("[preload] menu backdrop could not be composed — using the flat stage", error);
    }
}
