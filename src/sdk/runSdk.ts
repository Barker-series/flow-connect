...
import { createKvMirror } from "./kvMirror.ts";
...
export async function purchaseShopItem(itemId: string, idempotencyKey: string): Promise<ShopPurchaseResponse> {
    ...
    //
    // SDK 5.25+ RESOLVES a dismissed checkout as `{ cancelled: true }` instead
    // of rejecting. The purchase coordinator classifies cancellation from a
    // thrown error, so the resolved cancel is turned back into one here.
    const result = await withHostOverlay(() => RundotGameAPI.shop.purchase(itemId, idempotencyKey));
    if (result.cancelled) {
        const error = new Error("Checkout cancelled by player") as Error & { code: string };
...
/**
 * Wins banked toward the like prompt, `-1` once spent. Mirrored from
 * appStorage — browser storage is unavailable in the RUN iframe.
 */
export const likePromptCounter = createKvMirror("flowconnect:like-prompt:v1");
/** Wins to bank before asking. Nobody likes a game they just met. */
const LIKE_PROMPT_MIN_WINS = 3;

function likeCounterRead(): number {
    return Number(likePromptCounter.get() ?? "0") || 0;
}

function likeCounterWrite(value: number): void {
    likePromptCounter.set(String(value));
}
