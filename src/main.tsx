...
import { analytics, funnelMarks } from "./systems/analytics/analyticsConfig.ts";
...
async function boot() {
    ...
    await initSdk();
    // Small bookkeeping mirrors read synchronously later (funnel marks, the
    // like-prompt counter). Hydrate BEFORE the transport opens so buffered
    // once-ever funnel steps are deduped against what already fired.
    await Promise.all([funnelMarks.hydrate(), likePromptCounter.hydrate()]);
    ...
    registerLifecycles({
        ...
        onBackButton: () => {
            ...
            if (state.phase === "playing") {
                // Back from a board lands on the level list it was picked from.
                store.patch({
                    phase: "menu",
                    menuScreen: state.level?.kind === "daily" ? "main" : "levels",
                    paused: false,
                });
                void saveSystem.flush();
    ...
    analytics.funnelStep("flowconnect_first_level", 1);
    analytics.sessionStart(store.get().levelsCompleted === 0);
