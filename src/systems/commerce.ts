 ...
 * mapping from entitlement to tube set lives there and only there.
 */
...
const PRODUCT_NAMES: Readonly<Record<ProductId, string>> = {
    tube_pack: "TUBE PACK — NIGHTLIFE",
    ad_free: "AD-FREE FOREVER",
    power_pass: "POWER PASS",
};

const PRODUCT_DESCRIPTIONS: Readonly<Record<ProductId, string>> = {
    tube_pack: "Unlocks the Aurora and Synthwave tube sets permanently. Cosmetic only.",
    ad_free:
        "Removes the between-levels interstitial forever. Optional rewarded videos remain, and are never required.",
    power_pass: "Everything above, plus the Prism tubes — jewel tones on black glass, sold nowhere else.",
};
...
export function ownsAdFree(): boolean {
    ...
    const pass = products.get("power_pass");
...
export function productView(productId: ProductId): ProductView {
    ...
    const solved = store.get().levelsCompleted;
    const required = PRODUCT_UNLOCK_LEVELS[productId];
    const eligible = solved >= required;
    ...
    return {
        ...
        statusLabel: owned
            ? "OWNED"
            : !eligible
              ? `SOLVE ${required} LEVEL${required === 1 ? "" : "S"}`
              : devPreview
