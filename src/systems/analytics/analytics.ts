...
//     time it ever happens and never again, persisted through `marksStore`. Without
...
export interface AnalyticsConfig {
    ...
    /**
     * Where the once-ever marks persist. Browser storage is unavailable in the
     * RUN iframe, so the game injects a synchronous mirror of an appStorage key
     * (see sdk/kvMirror.ts). Without one, marks dedup for this page life only.
     */
    marksStore?: { get(): string | null; set(value: string): void; remove(): void };
...
export function createAnalytics(config: AnalyticsConfig): Analytics {
    ...
    const marksStore = config.marksStore ?? null;

    // Once-ever marks are read once and cached: funnelStep runs on gameplay
    // paths, and a storage read per step would be a needless hit.
    ...
    function persistMarks(): void {
        try {
            marksStore?.set(JSON.stringify([...readMarks()]));
        } catch {
    ...
    const system: Analytics = {
        ...
        funnelStep(funnel, step, props) {
            ...
            // The once-ever check runs at DELIVERY, not at call time: steps
            // raised before the SDK is up are buffered, and the persisted
            // marks are only readable once it is.
            deliver(() => {
