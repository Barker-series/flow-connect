...
const LEGACY_RETURN_REMINDER_ID = "flowconnect-return-reminder";
...
function normalize(values: Record<string, unknown>): RuntimeConfig {
    const root =
        values.flowconnect_runtime && typeof values.flowconnect_runtime === "object"
            ? (values.flowconnect_runtime as Record<string, unknown>)
            : values;
    return {
